(() => {
  ...
  const MUSIC_KEY = "rehDoggGpNeonRunMusic";
  ...
  const ui = {
    ...
    menuMusic: document.getElementById("menuMusicBtn"),
    restart: document.getElementById("restartBtn"),
    mainMenu: document.getElementById("mainMenuBtn"),
    pause: document.getElementById("pauseBtn"),
    pauseMusic: document.getElementById("pauseMusicBtn"),
  ...
  function createAudio() {
    ...
    let engineOsc;
    let engineGain;
    const music = new Audio("./soundtrack.mp3");
    music.loop = true;
    music.volume = .25;
    music.preload = "auto";
    music.addEventListener("canplaythrough", () => {
      state.musicReady = true;
      console.log("REH DOGG GP music ready: ./soundtrack.mp3");
    }, { once: true });
    music.addEventListener("error", () => {
      state.musicReady = false;
      console.warn("REH DOGG GP music could not load: ./soundtrack.mp3");
    }, { once: true });
    ...
    function updateMusic() {
      if (!music) return;
      if (state.running && !state.paused && state.musicOn) {
        music.muted = false;
        music.volume = .25;
        music.play()
          .then(() => console.log("REH DOGG GP music playing"))
          .catch(err => console.warn("REH DOGG GP music play blocked or failed:", err));
      } else {
        music.pause();
      }
    }
    ...
    function startMusicNow() {
      if (!state.musicOn) {
        console.log("REH DOGG GP music is OFF in saved preference");
        return;
      }
      music.muted = false;
      music.volume = .25;
      music.play()
        .then(() => console.log("REH DOGG GP music started from user action"))
        .catch(err => console.warn("REH DOGG GP music start failed:", err));
    }
  ...
  function setMusic(on) {
    ...
    updateMusicButtons();
    if (on && state.running && !state.paused) audio.startMusicNow();
    audio.updateMusic();
  }
  ...
  function updateMusicButtons() {
    const label = state.musicOn ? "Music On" : "Music Off";
    ui.menuMusic.textContent = label;
    ui.pauseMusic.textContent = label;
    ui.menuMusic.classList.toggle("off", !state.musicOn);
    ui.pauseMusic.classList.toggle("off", !state.musicOn);
  }
  ...
  function bindEvents() {
    ...
    ui.menuMusic.addEventListener("click", toggleMusic);
    ui.pauseMusic.addEventListener("click", toggleMusic);
