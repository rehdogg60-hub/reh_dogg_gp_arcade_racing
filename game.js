(() => {
  ...
  const ui = {
    ...
    cockpitView: document.getElementById("cockpitViewBtn"),
    chaseView: document.getElementById("chaseViewBtn"),
    ...
    dashLap: document.getElementById("dashLap"),
  ...
  function routeCenterAt(stage, distance) {
    ...
    const base =
      Math.sin(n * Math.PI * 2) * 9 +
      Math.sin(n * Math.PI * 6 + .8) * 4.5 +
      Math.sin(n * Math.PI * 10 + 1.6) * 1.8;
    return THREE.MathUtils.clamp(base, -15, 15);
  }

  function routeWidthAt(stage, distance) {
    const inNarrow = stage.name === "Volcano Circuit" && isInTunnel(stage, distance);
    return inNarrow ? 10 : 12;
  }
  ...
  function createCar() {
    ...
    const body = new THREE.Mesh(new THREE.BoxGeometry(4.9, .85, 6.9), red);
    body.castShadow = !isMobile;
    body.position.y = .78;
    car.add(body);

    const hood = new THREE.Mesh(new THREE.BoxGeometry(4.35, .42, 2.8), red);
    hood.position.set(0, 1.02, -2.6);
    hood.scale.set(.9, .8, 1);
    car.add(hood);

    const nose = new THREE.Mesh(new THREE.BoxGeometry(4.1, .52, 1.2), red);
    nose.position.set(0, .84, -4);
    car.add(nose);
    ...
    const cockpit = new THREE.Mesh(new THREE.BoxGeometry(2.55, .92, 2.15), glass);
    cockpit.position.set(0, 1.45, -.7);
    car.add(cockpit);

    const rearDeck = new THREE.Mesh(new THREE.BoxGeometry(4.8, .36, 2.1), red);
    rearDeck.position.set(0, 1.05, 2.35);
    car.add(rearDeck);

    const wing = new THREE.Mesh(new THREE.BoxGeometry(5.5, .24, .62), dark);
    wing.position.set(0, 1.72, 3.65);
    car.add(wing);
    ...
    [-1, 1].forEach(side => {
      ...
      const neonStripe = new THREE.Mesh(new THREE.BoxGeometry(.16, .1, 5.9), cyan);
      neonStripe.position.set(side * 1.82, 1.16, -.05);
      car.add(neonStripe);

      const goldSkirt = new THREE.Mesh(new THREE.BoxGeometry(.2, .26, 6.1), gold);
      goldSkirt.position.set(side * 2.52, .66, .05);
      car.add(goldSkirt);

      const headlight = new THREE.Mesh(new THREE.BoxGeometry(.72, .2, .12), head);
      headlight.position.set(side * 1.25, .92, -4.64);
      car.add(headlight);
  ...
  function buildStage(index) {
    ...
    const roadLight = new THREE.PointLight(0xffffff, isMobile ? .65 : 1, 70);
    roadLight.position.set(0, 12, 10);
    trackGroup.add(roadLight);
    ...
    for (let i = 0; i < segmentCount; i++) {
      ...
      const roadMat = makeMat(stage.road, 0x111111, .5);
      const road = new THREE.Mesh(new THREE.BoxGeometry(12, .08, 52), roadMat);
      ...
      [-1, 1].forEach(side => {
        const rail = new THREE.Mesh(new THREE.BoxGeometry(.36, .5, 50), makeMat(stage.edge, stage.edge));
        rail.position.set(side * 6.8, .32, 0);
        segment.add(rail);
      });

      const stripe = new THREE.Mesh(new THREE.BoxGeometry(.28, .1, 8), makeMat(0xffffff, stage.accent));
  ...
  function buildProps(stage) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      prop.userData.roadsideOffset = 15 + (i % 3) * 3;
  ...
  function buildRouteFeatures(stage) {
    ...
    stage.tunnels.forEach(([start, end]) => {
      ...
      sign.userData.side = -1;
      sign.userData.roadsideOffset = 13.5;
      sign.position.y = 6.4;
      trackGroup.add(sign);
      featureObjects.push(sign);
    });

    stage.signs.slice(1, 3).forEach((text, index) => {
      const sign = makeTextSign(text, index === 1 ? "#00e5ff" : "#ffdf55");
      sign.userData.distance = stage.length * (.28 + index * .18);
      sign.userData.routeFeature = true;
      sign.userData.side = index % 2 ? 1 : -1;
      sign.userData.roadsideOffset = 16;
      sign.position.y = 6.2;
      trackGroup.add(sign);
      featureObjects.push(sign);
    });
  ...
  function createGate(stage, label, color, checker) {
    ...
    [-1, 1].forEach(side => {
      const post = new THREE.Mesh(new THREE.BoxGeometry(.55, 6, .55), mat);
      post.position.set(side * 7.2, 3, 0);
      g.add(post);
    });
    const top = new THREE.Mesh(new THREE.BoxGeometry(14.8, .55, .55), mat);
    top.position.y = 6;
    g.add(top);
    const sign = makeTextSign(label, checker ? "#ffffff" : "#ffdf55");
    sign.position.y = 7.2;
    sign.scale.setScalar(.82);
    g.add(sign);
    if (checker) {
  ...
  function createTunnelArch(stage) {
    ...
    [-1, 1].forEach(side => {
      const wall = new THREE.Mesh(new THREE.BoxGeometry(1.1, 5.5, 4.5), mat);
      wall.position.set(side * 7.3, 2.75, 0);
      g.add(wall);
    });
    const roof = new THREE.Mesh(new THREE.BoxGeometry(15.5, 1, 4.5), mat);
  ...
  function setCameraView(view) {
    race.cameraView = view;
    document.body.classList.toggle("cockpit-view", view === "cockpit");
    ui.cockpitView.classList.toggle("selected", view === "cockpit");
    ui.chaseView.classList.toggle("selected", view === "chase");
    if (car) car.visible = view !== "cockpit";
  }
  ...
  function startRace(stageIndex = race.stage) {
    ...
    car.visible = race.cameraView !== "cockpit";
  ...
  function addEvents() {
    ...
    ui.cockpitView.addEventListener("click", () => { audio.unlock(); setCameraView("cockpit"); });
    ui.chaseView.addEventListener("click", () => { audio.unlock(); setCameraView("chase"); });
  ...
  function updateRace(dt) {
    ...
    const roadHalf = routeWidthAt(stage, race.lapProgress) / 2 - 1.35;
  ...
  function updateWorldScroll(distance) {
    ...
    propGroup.children.forEach(obj => {
      ...
      obj.visible = rel < (race.cameraView === "cockpit" ? 420 : 620);
    });
    ...
    featureObjects.forEach(obj => {
      ...
      const rel = ((obj.userData.distance - race.lapProgress + stage.length) % stage.length);
      obj.position.z = -rel * visibleScale;
      obj.position.x = routeCenterAt(stage, routeDistance) + (obj.userData.side ? obj.userData.side * obj.userData.roadsideOffset : 0);
      obj.rotation.y = THREE.MathUtils.clamp((routeCenterAt(stage, routeDistance + 80) - routeCenterAt(stage, routeDistance)) * .018, -.42, .42);
      obj.visible = rel < (race.cameraView === "cockpit" ? 500 : 650) || rel > stage.length - 80;
    });
  ...
  function updateCamera(dt) {
    const stage = stages[race.stage];
    const tunnel = isInTunnel(stage, race.lapProgress);
    const shake = Math.max(0, race.speed - 125) / 120 * (isMobile ? .05 : .08);
    const aheadCenter = routeCenterAt(stage, race.lapProgress + 190);
    if (race.cameraView === "cockpit") {
      const carPosition = car.position;
      const carAngle = car.rotation.y;
      const lookDistance = tunnel ? 20 : 24;
      const target = new THREE.Vector3(
        carPosition.x,
        carPosition.y + (tunnel ? 1.7 : 2.05),
        carPosition.z + .4
      );
      camera.position.lerp(target, 1 - Math.pow(.02, dt));
      camera.position.x += (Math.random() - .5) * shake;
      camera.position.y += (Math.random() - .5) * shake;
      camera.lookAt(
        THREE.MathUtils.lerp(carPosition.x + Math.sin(carAngle) * lookDistance, aheadCenter, .65),
        carPosition.y + (tunnel ? 1.55 : 1.75),
        carPosition.z - Math.cos(carAngle) * lookDistance
      );
      return;
    }

    const target = new THREE.Vector3(
      THREE.MathUtils.lerp(car.position.x, aheadCenter, .24),
      (tunnel ? 6.8 : 8.8) + race.speed * (tunnel ? .004 : .007),
      (tunnel ? 11 : 13.5) + race.speed * (tunnel ? .006 : .01)
    );
    camera.position.lerp(target, 1 - Math.pow(.035, dt));
    camera.position.x += (Math.random() - .5) * shake;
    camera.position.y += (Math.random() - .5) * shake;
    camera.lookAt(aheadCenter, tunnel ? 1.5 : 1.3, -34);
  }
  ...
  function updateHud() {
    ...
    ui.dashSpeed.textContent = `${Math.round(Math.max(0, race.speed))}`;
    ui.dashLap.textContent = `${race.lap} / ${LAP_GOAL}`;
  }
